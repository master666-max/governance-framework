...
> **本仓是模块发布面（公开）**：治理核（audit-kit）＋ 演化核（evocore）＋ 记忆库（memsys，当前宿主实例）＋
> 微内核（evo-seat）＋ 装配器（build）＋ 门与对账面（tools）＋ 发布面（skill）。
> 纯 Python 标准库，**零第三方依赖**；各模块自带 unittest 测试套件。许可证：**MIT**（见 `LICENSE`）。
...
### 宿主的本质：可替换，且正因为可替换才有治理

**宿主＝被治理的执行体**：真正持有内容、动手写入的那一层。记忆库（memsys）不是体系
本身，它只是**当前接入的宿主实例**——第一个装配示例。

治理双核对宿主**一无所知**，只通过四个接触点与它相连：
① 写前出示**凭证**（治理核签发）；② 内容过**指纹**（演化核计算）；
③ 事件进**账本**（治理核追加）；④ 状态可**对拍**（门与对账面复核）。

⇒ **通用性**：任何能满足这四个接触点的系统——LLM wiki、工件库、数据库、工作流审计、
任意智能体的知识仓——都能当宿主。
⇒ **可替换性**：换宿主，双核零改动、门面零改动（至多多一个适配件），治理强制力原样
生效。治理位在库外、不寄生于宿主内部——这就是换宿主的代价为什么这么低。

### 不同宿主的判据适配（接入草案）

四接触点通用，**判据按宿主族适配**——同一接触点在不同宿主上落成不同的具体判据：

| 宿主族 | ①凭证（写前出示） | ②指纹（内容锚） | ③账本（事件链） | ④对拍（状态可验） |
|---|---|---|---|---|
| **记忆库**（memsys，现役） | 表级 Capability | content_hash 64hex（排除易变字段） | 事件链 prev-hash | 双库同链对拍＋注册表对拍 |
| **LLM wiki**（词条协作） | 词条域签发（页面级） | 词条正文指纹（排除修订元数据） | 修订事件链 | 词条↔修订史互证＋引用图无环 |
| **工件库**（代码/资产） | 路径域签发（glob） | 文件 sha256＋产物聚合哈希 | 提交即账＋准入事件链 | 三态指纹（工作树/索引/HEAD）＋副本族一致 |
| **数据库**（结构化状态） | 表/行级 scope | 行规范序列化哈希（排除易变列） | 审计表触发器 append-only（现成平移） | 快照双源对拍 |
| **工作流审计**（流程系统） | 阶段门令（转移须持对应阶段凭证） | 任务状态＋产出物指纹 | 转移事件链 | 流程定义↔实际转移序列对拍 |

> 本表是**接入判据草案**（宿主设计指引），非实测结论；任一宿主接入 = 四接触点接线 +
> 本表适配 + 双对照验证（正对照误拒 0 / 负对照必红）三步走。
...
**演化核管记忆的生命，治理核管行为的账，记忆库只是当前宿主——宿主可换、治理不换；装配器证明能装成四种形态，门与对账面负责怀疑一切。**
