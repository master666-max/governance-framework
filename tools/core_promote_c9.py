# -*- coding: utf-8 -*-
"""core_promote_c9.py — 第九圈扩核预演：passA(core9 四盲批 390) × 九圈账本新增 356 行 交集
...
BASE = 10925          # 作者时点账本行数-356（冻结于 20260924 修复）
if len(led) < BASE: raise SystemExit(f"账本({len(led)})短于固化基线10925——数据异常，拒绝重放")
if len(led) > BASE + 356: print(f"⚠ 账本已增至{len(led)}行：重放窗口钉死在作者时点(+356)，其后的行不在本预演内")
new = led[BASE:]

passA = []
for b in ("core9-passA-a", "core9-passA-b1", "core9-passA-b2", "core9-passA-b3"):
...
for q in passA:
    ...
    if ln is None or q["start_anchor"] not in ln or q["end_anchor"] not in ln:
        af += 1
print(f"passA 锚点自验(c9): {len(passA)-af}/{len(passA)} pass")
...
out = os.path.join(AUD, "imap2", "data", "CORE-LEDGER-C9-预演.csv")
...
print(f"匹配 {len(matched)}（账本未配 {356-len(matched)} / passA 未配 {len(passA)-len(matched)}）")
