# skill-installer README（自演化知识库构建体系 **v1.0.0**，2026-09-21）

> 沿革：本件源于 G-03 原型 v0（20260919）；v1.0.0 起随包发布，本节标题口径与包版本对齐。
...
> 逐条为原型期登记。其中 ①（存量迁移）与 ③（许可隔离声明）在 v1.0.0 包内**已有对应件**
> （`migrate.py` / `LICENSE-NOTICE.md`）。
> **用户裁决（2026-09-21）：不注销，四条全部保留原样**——"包内已有对应件"不等于"原型期限制已消失"。
